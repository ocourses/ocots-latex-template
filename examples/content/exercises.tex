...
\begin{lstlisting}
\begin{exercise}
    Un exercice simple, sans etiquette ni bareme.
\solution
    Son corrige.
\end{exercise}
\end{lstlisting}
...
\begin{exercise}[label=ex:matrices, points=4]
    Un exercice etiquete et bareme. On le cite par \verb|\ref{ex:matrices}|.
    Calculer $e^{tA}$ pour $A = \begin{pmatrix} 0 & 1 \\ -1 & 0\end{pmatrix}$.
\solution
    On part de la definition $e^{tA} = \sum_{n \ge 0} \frac{t^n}{n!} A^n$ et de
    l'identite $A^2 = -I$, d'ou
    \[
...
\begin{exercise}[nosolution]
    Un exercice sans corrige, quel que soit le mode~: la clef
...
\begin{lstlisting}
\begin{exercise*}
    Un exercice non numerote : aucun compteur n'est incremente.
\solution
    Son corrige, compose seulement en mode inline.
\end{exercise*}
\end{lstlisting}
\begin{exercise*}
    Un exercice non numéroté : aucun compteur n'est incrémenté, et l'exercice
    suivant reprend la numérotation là où elle en était. Sans numéro, le
    corrigé ne peut pas être reporté en fin de partie : il n'est composé qu'en
    mode \texttt{inline}.
\solution
    Son corrigé, composé seulement en mode \texttt{inline}.
\end{exercise*}
...
\begin{exercise}[points=6]
    \begin{question}
        Une premiere question, numerotee dans l'exercice.
    \end{question}
    \begin{question}
    ...
    \end{question}
\solution
    Le compteur de questions repart a zero dans le corrige~; \verb|\newquestion|
    permet de le faire avancer a la main quand le corrige ne suit pas l'enonce
    question par question.
    \begin{question}
        Corrige de la premiere question.
    \end{question}
    \begin{question}
        Corrige de la deuxieme.
    \end{question}
\end{exercise}
\end{lstlisting}
\begin{exercise}[points=6]
...
\begin{lstlisting}
\begin{correction}
    Une correction breve, au fil du texte.
\end{correction}
\end{lstlisting}
\begin{correction}
