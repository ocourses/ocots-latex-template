...
\begin{lstlisting}
\begin{proposition}[title={Une proposition a demontrer}, label=prop:ademontrer]%
    Pour tout $x \in \R$, $x^2 \ge 0$.
\end{proposition}
\end{lstlisting}
\begin{proposition}[title={Une proposition à démontrer}, label=prop:ademontrer]%
    Pour tout $x \in \R$, $x^2 \ge 0$.
\end{proposition}
...
\begin{proof}
    ...
    couleur du texte.
    \newstep Sans titre, \verb|\newstep| ouvre seulement un nouveau paragraphe.
\end{proof}
\end{lstlisting}
\begin{proof}
...
\begin{lstlisting}
\begin{proof}
    \newstep[Existence] En tete de preuve, le titre suit directement le
    marqueur.
    \newstep[Unicite] Chaque etape s'ouvre sur un nouveau paragraphe, son titre
    en italique, suivi d'un point ajoute automatiquement.
    \newstep[Point ii)] Un titre qui finit par une parenthese recoit aussi son
    point.
\end{proof}
\end{lstlisting}
\begin{proof}
    \newstep[Existence] En tête de preuve, le titre suit directement le
    marqueur.
    \newstep[Unicité] Chaque étape s'ouvre sur un nouveau paragraphe, son titre
    en italique, suivi d'un point ajouté automatiquement.
    \newstep[Point ii)] Un titre qui finit par une parenthèse reçoit aussi son
    point.
\end{proof}

Une preuve ou un exemple qui se termine par une liste ou par une équation hors
texte place son symbole de fin avec \verb|\qedhere|, sans quoi il passerait
...
\begin{lstlisting}
\begin{proof}
    La preuve se termine par une equation :
    \[ x^2 \ge 0. \qedhere \]
\end{proof}
\end{lstlisting}
\begin{proof}
    La preuve se termine par une équation :
    \[ x^2 \ge 0. \qedhere \]
\end{proof}
...
Une note appelée dans une boîte se compose en deux temps : \verb|\footnotemark|
dans la boîte, \verb|\footnotetext| juste après. Placé entre l'énoncé et sa
preuve, le \verb|\footnotetext| ne change pas l'espace qui les sépare.

\begin{lstlisting}
\begin{proposition}
    Pour tout $x \in \R$, $\lvert x\rvert^2 = x^2$\footnotemark{}.
\end{proposition}
\footnotetext{Ou $\lvert x\rvert = \max(x, -x)$.}
\begin{proof}
    Distinguer les cas $x \ge 0$ et $x < 0$.
\end{proof}
\end{lstlisting}
\begin{proposition}
    Pour tout $x \in \R$, $\lvert x\rvert^2 = x^2$\footnotemark{}.
\end{proposition}
\footnotetext{Où $\lvert x\rvert = \max(x, -x)$.}
\begin{proof}
    Distinguer les cas $x \ge 0$ et $x < 0$.
\end{proof}
